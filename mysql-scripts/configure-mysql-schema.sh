...
dbName=$(ctx instance properties dbName)
...
dbUser=$(ctx instance properties dbUserName)
ctx logger info "${currHostName}:${currFilename} :dbUser is ${dbUser}"

dbPassW=$(ctx instance properties dbUserPassword)
...
export currSchema=$(ctx instance properties schemaurl)
...
export dbQuery=$(ctx instance properties query)
