...
dbUser=$(ctx node properties dbUserName)
ctx logger info "${currHostName}:${currFilename} :dbUser is ${dbUser}"

dbPassW=$(ctx node properties dbUserPassword)
...
export currSchema=$(ctx node properties schemaurl)
...
export dbQuery=$(ctx node properties query)
